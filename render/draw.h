...
#define fur_render_rect(render, ...) \
    IMPL_fur_render_rect((render), STRUCT_LIT(OP_fur_render_rect){ .target = render->defTarget, .pos = STRUCT_LIT(v2){0,0}, .size = STRUCT_LIT(v2){0,0}, .col = STRUCT_LIT(v4){1,1,1,1}, .transf = mat4_identity, __VA_ARGS__ })

#define fur_render_tex(render, ...) \
    IMPL_fur_render_tex((render), STRUCT_LIT(OP_fur_render_tex){ .target = render->defTarget, .texture = NULL, .pos = STRUCT_LIT(v2){0,0}, .size = STRUCT_LIT(v2){NAN,NAN}, .col = STRUCT_LIT(v4){1,1,1,1}, .transf = mat4_identity, .sample = STRUCT_LIT(v4){NAN,NAN,NAN,NAN}, __VA_ARGS__ })

#define fur_render_renderTarget(render, ...) \
    IMPL_fur_render_renderTarget((render), STRUCT_LIT(OP_fur_render_renderTarget){ .out_target = render->defTarget, .in_target = NULL, .pos = STRUCT_LIT(v2){0,0}, .size = STRUCT_LIT(v2){NAN,NAN}, .col = STRUCT_LIT(v4){1,1,1,1}, .transf = mat4_identity, .sample = STRUCT_LIT(v4){NAN,NAN,NAN,NAN}, __VA_ARGS__ })
